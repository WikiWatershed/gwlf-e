    ...
    for Y in range(z.NYrs):
        ...
        for i in range(12):
            ...
            for j in range(z.DaysMonth[Y,i]):
                ...
                #for l in range(z.NLU):
                z.ImpervAccum[:] = (z.ImpervAccum * np.exp(-0.12) +
                                    (1 / 0.12) * (1 - np.exp(-0.12)))
                z.PervAccum[:] = (z.PervAccum * np.exp(-0.12) +
                                  (1 / 0.12) * (1 - np.exp(-0.12)))
...
#         if self.times[name] is not None:
#             log.warn("timer {!s} is already running!".format(name))
#             return
#         self.times[name] = time.time()
...
#         name = name if self.use_name is None else self.use_name
#         if name is None:
#             log.warn("timer name was never set!")
#             return
...
#                 # re-initialized to a default value at some point?
#                 # for l in range(z.NLU):
#                 z.ImpervAccum[:] = (z.ImpervAccum * np.exp(-0.12) + (1 / 0.12) * (1 - np.exp(-0.12)))
#                 z.PervAccum[:] = (z.PervAccum * np.exp(-0.12) + (1 / 0.12) * (1 - np.exp(-0.12)))
...
#                 if z.ETFlag is ETflag.HAMON_METHOD:
#                     if z.DailyTemp > 0:
#                         z.SatVaPressure = (33.8639 * ((0.00738 * z.DailyTemp + 0.8072) ** 8 - 0.000019 * np.absolute(
#                             1.8 * z.DailyTemp + 48) + 0.001316))
#                         z.PotenET = (0.021 * z.DayHrs[i] ** 2 * z.SatVaPressure / (z.DailyTemp + 273))
#                         z.ET = z.KV[i] * z.PotenET * z.PcntET[i]
...
#                 # CALCULATE DAILY NUTRIENT LOAD FROM PONDING SYSTEMS
#                 z.PondNitrLoad = (z.NumPondSys[i] * (z.NitrSepticLoad - z.NitrPlantUptake * grow_factor))
#                 z.PondPhosLoad = (z.NumPondSys[i] * (z.PhosSepticLoad - z.PhosPlantUptake * grow_factor))
...
#                 # Obtain the monthly Normal Nitrogen
#                 z.MonthNormNitr[i] = (z.MonthNormNitr[i] + z.NitrSepticLoad - z.NitrPlantUptake * grow_factor)

#                 # 0.56 IS ATTENUATION FACTOR FOR SOIL LOSS
#                 # 0.66 IS ATTENUATION FACTOR FOR SUBSURFACE FLOW LOSS
#                 z.MonthShortNitr[i] = (z.MonthShortNitr[i] + z.NitrSepticLoad - z.NitrPlantUptake * grow_factor)
#                 z.MonthShortPhos[i] = (z.MonthShortPhos[i] + z.PhosSepticLoad - z.PhosPlantUptake * grow_factor)
#                 z.MonthDischargeNitr[i] = z.MonthDischargeNitr[i] + z.NitrSepticLoad
#                 z.MonthDischargePhos[i] = z.MonthDischargePhos[i] + z.PhosSepticLoad

#             # CALCULATE WITHDRAWAL AND POINT SOURCE FLOW VALUES
#             z.Withdrawal[Y, i] = (z.Withdrawal[Y, i] + z.StreamWithdrawal[i] + z.GroundWithdrawal[i])
#             z.PtSrcFlow[Y, i] = z.PtSrcFlow[Y, i] + z.PointFlow[i]

#             # CALCULATE THE SURFACE RUNOFF PORTION OF TILE DRAINAGE
#             z.TileDrainRO[Y, i] = (z.TileDrainRO[Y, i] + [z.AgRunoff[Y, i] * z.TileDrainDensity])

#             # CALCULATE SUBSURFACE PORTION OF TILE DRAINAGE
#             if z.AreaTotal > 0:
#                 z.GwAgLE[Y, i] = (z.GwAgLE[Y, i] + (z.GroundWatLE[Y, i] * (z.AgAreaTotal / z.AreaTotal)))
#             z.TileDrainGW[Y, i] = (z.TileDrainGW[Y, i] + [z.GwAgLE[Y, i] * z.TileDrainDensity])

#             # ADD THE TWO COMPONENTS OF TILE DRAINAGE FLOW
#             z.TileDrain[Y, i] = (z.TileDrain[Y, i] + z.TileDrainRO[Y, i] + z.TileDrainGW[Y, i])
...
#     for i in range(12):
#         z.AvStreamFlow[i] = (
#                     z.AvRunoff[i] + z.AvGroundWater[i] + z.AvPtSrcFlow[i] + z.AvTileDrain[i] - z.AvWithdrawal[i])
...
#     z.AvStreamFlowSum = (z.AvRunoffSum + z.AvGroundWaterSum + z.AvPtSrcFlowSum + z.AvTileDrainSum - z.AvWithdrawalSum)
